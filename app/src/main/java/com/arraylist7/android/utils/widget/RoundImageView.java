...
public class RoundImageView extends NetworkImageView {
    ...
    private int leftButtomRadius;
    private int rightButtomRadius;
    private int type;
    ...
    private void inital(Context context, AttributeSet attrs) {
        ...
        borderRadius = dp2px(array.getDimension(R.styleable.RoundImageView_allRadius, BORDER_RADIUS_DEFAULT));
        leftTopRadius = dp2px(array.getDimension(R.styleable.RoundImageView_leftTopRadius, BORDER_RADIUS_DEFAULT));
        rightTopRadius = dp2px(array.getDimension(R.styleable.RoundImageView_rightTopRadius, BORDER_RADIUS_DEFAULT));
        leftButtomRadius = dp2px(array.getDimension(R.styleable.RoundImageView_leftBottomRadius, BORDER_RADIUS_DEFAULT));
        rightButtomRadius = dp2px(array.getDimension(R.styleable.RoundImageView_rightButtomRadius, BORDER_RADIUS_DEFAULT));
    ...
    private void setBitmapShader() {
        ...
        float scale1 = 1.0f;
        float scale2 = 1.0f;
        final boolean fits = (drawableWidth < 0 || viewWidth == drawableWidth) && (drawableHeight < 0 || viewHeight == drawableHeight);
        if (type == TYPE_CIRCLE) {
        ...
        if (drawableWidth <= 0 || drawableHeight <= 0) {
            ...
            if (ScaleType.MATRIX == getScaleType()) {
                ...
                if (null == matrix)
                    matrix = new Matrix();
                matrix.setTranslate(Math.round((viewWidth - drawableWidth) * 0.5f), Math.round((viewHeight - drawableHeight) * 0.5f));
                ...
                if (drawableWidth <= viewWidth && drawableHeight <= viewHeight) {
                    ...
                    scale = Math.min((float) viewWidth / (float) drawableWidth, (float) viewHeight / (float) drawableHeight);
                }
