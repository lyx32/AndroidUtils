...
package com.arraylist7.android.utils.widget.v4;
...
public class SwipeRefreshLayout extends ViewGroup {
	...
	private int REFRESH_TRIGGER_DISTANCE = 120;
	...
	private int mProgressBarTopHeight;
	private int mProgressBarBottomHeight;
	...
	public SwipeRefreshLayout(Context context, AttributeSet attrs) {
		...
		mProgressBarTopHeight = (int) (metrics.density * PROGRESS_BAR_HEIGHT);
		mProgressBarBottomHeight = (int) (metrics.density * PROGRESS_BAR_HEIGHT);
		...
		a.recycle();
		int loading1 = android.R.color.holo_blue_light;
		int loading2 = android.R.color.holo_green_light;
		int loading3 = android.R.color.holo_orange_light;
		int loading4 = android.R.color.holo_red_light;
		final TypedArray b = context.obtainStyledAttributes(attrs, R.styleable.SwipeRefreshLayoutLoadingColor);
		int allColor = b.getColor(R.styleable.SwipeRefreshLayoutLoadingColor_allColor, -1);
		int topColor = b.getColor(R.styleable.SwipeRefreshLayoutLoadingColor_topColor, -1);
		int topColor1 = b.getColor(R.styleable.SwipeRefreshLayoutLoadingColor_topColor1, loading1);
		int topColor2 = b.getColor(R.styleable.SwipeRefreshLayoutLoadingColor_topColor2, loading2);
		int topColor3 = b.getColor(R.styleable.SwipeRefreshLayoutLoadingColor_topColor3, loading3);
		int topColor4 = b.getColor(R.styleable.SwipeRefreshLayoutLoadingColor_topColor4, loading4);
		int bottomColor = b.getColor(R.styleable.SwipeRefreshLayoutLoadingColor_bottomColor, -1);
		int bottomColor1 = b.getColor(R.styleable.SwipeRefreshLayoutLoadingColor_bottomColor1, loading1);
		int bottomColor2 = b.getColor(R.styleable.SwipeRefreshLayoutLoadingColor_bottomColor2, loading2);
		int bottomColor3 = b.getColor(R.styleable.SwipeRefreshLayoutLoadingColor_bottomColor3, loading3);
		int bottomColor4 = b.getColor(R.styleable.SwipeRefreshLayoutLoadingColor_bottomColor4, loading4);
		boolean isSetColor = false;
		if(-1 != allColor){
			topColor = bottomColor = allColor;
			setColor(allColor, allColor, allColor, allColor);
			isSetColor = true;
		}else{
			if(-1 != topColor){
				setTopColor(topColor, topColor, topColor, topColor);
				isSetColor = true;
			}
			if(-1 != bottomColor){
				setBottomColor(bottomColor, bottomColor, bottomColor, bottomColor);
				isSetColor = true;
			}
		}
		if(!isSetColor){
			setTopColor(topColor1, topColor2, topColor3, topColor4);
			setBottomColor(bottomColor1, bottomColor2, bottomColor3, bottomColor4);
		}
		b.recycle();
	}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		mProgressBar.setBounds(0, 0, width, mProgressBarTopHeight);
		if (getChildCount() == 0) {
			return;
		}
		...
		mProgressBarBottom.setBounds(0, height - mProgressBarBottomHeight, width, height);
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (10 >= getChildAt(0).getMeasuredHeight()) {
				return false;
			}
	...
	public void setProgressBarHeight(int mProgressBarHeight) {
		setProgressBarTopHeight(mProgressBarHeight);
		setProgressBarBottomHeight(mProgressBarHeight);
	}

	/**
	 * 设置头部进度条高度
	 * 
	 * @param mProgressBarTopHeight
	 */
	public void setProgressBarTopHeight(int mProgressBarTopHeight) {
		this.mProgressBarTopHeight = mProgressBarTopHeight;
	}

	/**
	 * 设置底部进度条高度
	 * 
	 * @param mProgressBarBottomHeight
	 */
	public void setProgressBarBottomHeight(int mProgressBarBottomHeight) {
		this.mProgressBarBottomHeight = mProgressBarBottomHeight;
	}
